  ...
  return (
    <div className="p-5 sm:p-6">
      <div className="flex items-center gap-4 mb-6 pb-5 border-b border-black/5 dark:border-white/10">
        ...
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Widad Amrani</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">Full Stack Developer</p>
          <p className="text-xs text-[#86198f] dark:text-[#c084fc] mt-1 font-medium">Available for new projects</p>
        </div>
      </div>

      <div className="space-y-3 mb-6">
        {[
          { icon: FiMail, label: 'Email', value: contactDetails.email, href: `mailto:${contactDetails.email}` },
          { icon: FiPhone, label: 'Phone', value: contactDetails.phone, href: `tel:${contactDetails.phone}` },
          { icon: FiMapPin, label: 'Location', value: contactDetails.location, href: null },
        ].map((row) => (
